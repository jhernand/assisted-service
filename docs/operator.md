...
- The assisted-service pod converts the registries.conf into an `ImageDigestMirrorSet` file, and use it with the `--idsm-file`[^icsp_deprecated_2] flag in a few `oc adm` commands run against the release image (executed from within the assisted-service pod itself).
